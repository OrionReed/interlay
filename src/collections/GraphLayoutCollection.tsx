...
export class GraphLayoutCollection extends BaseCollection {
  ...
  override onAdd(shapes: TLShape[]) {
    for (const shape of shapes) {
      ...
      this.addGeo(shape);
      for (const arrow of this.editor.getArrowsBoundTo(shape.id)) {
        this.addArrow(this.editor.getShape(arrow.arrowId) as TLArrowShape)
      }
    }
    this.refreshGraph();
  }
  ...
  override onRemove(shapes: TLShape[]) {
    ...
    for (const shape of shapes) {
      for (const arrow of this.editor.getArrowsBoundTo(shape.id)) {
        this.colaLinks.delete(arrow.arrowId)
      }
  ...
  addArrow = (arrow: TLArrowShape) => {
    ...
    if (!this.colaNodes.has(source.id) || !this.colaNodes.has(target.id)) return
