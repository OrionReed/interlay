...
const CollectionProvider: React.FC<CollectionProviderProps> = ({ editor, collections: collectionClasses, children }) => {
  const [collections, setCollections] = useState<Map<string, BaseCollection> | undefined>(undefined);
  ...
  useEffect(() => {
    if (editor) {
      ...
      CollectionsService.setCollections(initializedCollections);
    }
  ...
  const value = useMemo(() => ({
    ...
    getAll: () => collections ? [...collections.values()] : [],
  }), [collections]);
